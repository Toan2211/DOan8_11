...
def build_agent(output, platform):
    prog_name = os.path.basename(output) # = output
    ...
    working_dir = os.path.join(tempfile.gettempdir(), config.AGENT_NAME) # aa/bb/agent/driver
    #C:\Users\Admin\AppData\Local\Temp ---------------- Driver
    if os.path.exists(working_dir): #ktra dg` dan co chưa
        shutil.rmtree(working_dir)# xoa Driver

    agent_dir = os.path.dirname(os.path.abspath(__file__)) # c:\Users\Admin\OneDrive - The University of Technology\Desktop\Botnet8_11\agent  
    shutil.copytree(agent_dir, working_dir) # sao chep dir agent -> working
    ...
    shutil.rmtree(working_dir) #xoa thu muc -> aaa/bb/cc -> xoa cc
